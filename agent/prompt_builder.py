...
KANBAN_GUIDANCE = (
    ...
    "\n"
    "- **Workspace and Git boundary.** `cd $HERMES_KANBAN_WORKSPACE` first. "
    "For a `worktree` task, edit and test files but do not create worktrees, "
    "change refs/config/hooks, commit, push, or open a PR yourself. Shared Git "
    "metadata and network are intentionally unavailable. Your final "
    "`kanban_complete` stages a machine-readable handoff; after this model turn "
    "returns, a trusted host broker validates the exact active claim, board "
    "project, workspace, and `$HERMES_KANBAN_BRANCH`, commits locally, and "
    "parks the task at `AWAITING_TRUSTED_PUBLISHER v1`. Publishing is performed "
    "by a separate credentialed control plane.\n"
