...
KANBAN_GUIDANCE = (
    ...
    "- **Test isolation is mandatory.** `HERMES_HOME` alone does not isolate "
    "Kanban beneath `~/.hermes`. Before write tests, pin "
    "`HERMES_KANBAN_DB`, `HERMES_KANBAN_HOME`, and "
    "`HERMES_KANBAN_WORKSPACES_ROOT` to a temp root outside `~/.hermes` and "
    "assert the DB is not shared. Never run hammer/load fixtures against the "
    "shared board.\n"
