    ...
    def handle_tool_call(self, tool_name: str, args: dict, **kwargs) -> str:
        if tool_name == "hindsight_retain":
            ...
            try:
                ...
                resp = self._run_hindsight_operation(
                    lambda client: client.aretain_batch(bank_id=bank_id, items=[item],
                                                        retain_async=retain_async)
                )
                # As in auto-retain: an async write is only accepted here, so
                # track its operation id(s) for the next-turn read-after-write
                # wait before recall.
                if retain_async:
                    self._track_retain_ops(resp, bank_id)
